...
void SensorDisplay::updateMagPlot(MagData magData) {

    // Set time stamp (in sec.)
    if (startTime == -1) {
        startTime = magData.time;
    }

    double time = (QDateTime::currentDateTime().toMSecsSinceEpoch() - startTime) / 1000.0;

    // Update graph
    for (int i = 0; i < NUM_OF_SENSORS; i++)
    {
        int x = magData.packet.at(i*3);
        int y = magData.packet.at(i*3 + 1);
        int z = magData.packet.at(i*3 + 2);
